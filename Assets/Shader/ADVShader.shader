	...
	Properties
	{
		...
		SubShader
		{
			...
			half4 LightingADVToon(SurfaceOutput s, half3 lightDir, half atten) {
				...
				fixed4 d = tex2D(_ColorRamp, float2(1 - (atten * NdotL), 1));
				half4 c;
				...
				/*if (_WorldSpaceLightPos0.w == 0) {
					c.rgb = ((((NdotL)* d) * 2) * s.Albedo * _HighlightIntensity) + (1 - NdotL) * (_Ambient * s.Albedo);

					c.a = s.Alpha;
				}
										else {
										c.rgb = ((atten * NdotL) * d * s.Albedo * _HighlightIntensity) + (1 - atten * NdotL) * (_Ambient * s.Albedo);
										c.a = s.Alpha;
							}
